...
hypothesis: "mesclun-max and carrot max mix; exhaust the highest marginal profit crops up until their limits"
---
...
## Hypothesis
My hypothesis is that the mesclun would take up the largest number of beds due to its higher price per bed and lowest diminishing returns rate, then carrots, then tomatoes (~45% beds of mesclun = 30, ~30% beds of carrots = 20, ~25% beds of tomatoes = 10). Tomatoes would take the least number of beds because of its high diminishing returns, high labor rates, and high cost of fertilizer per bed. The mechanism that I would use is optimize P = MC, which means to allocate the next bed to whichever crop currently has the highest marginal profit (price minus its MC at that bed count) and continuing until we run out of beds.


## How I would know I was wrong
- Carrots end up with more beds than mesclun (even if tomatoes are still last). That would mean mesclun's price-per-bed edge doesn't survive its own diminishing returns curve
...
- Mesclun's marginal per-bed return crosses below carrots' before mesclun reaches 30 beds. Without a specific crossover point, "mesclun wins because lowest diminishing returns" can absorb almost any result.
- Total labor hours or total bed count (whichever cap binds first) turns out to reward a more even split across all three crops rather than a lopsided mesclun-heavy one
